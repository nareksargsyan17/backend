import { Body, Controller, Delete, Get, HttpCode, HttpStatus, Inject, Param, Post, Put, Query, Req, Res, SetMetadata, UploadedFile, UseGuards, UseInterceptors } from '@nestjs/common';
...
export class ApiGatewayController {
  constructor(
    ...
    @Post('film/create')
    @ApiResponse({ status: 201, description: 'The record has been successfully created.', schema : {allOf : [{$ref : getSchemaPath(CreateFilm)}]}})
    @UseGuards(AuthGuard)
    @SetMetadata('role', 1)
    @UseInterceptors(
    ...
    async create(
      ...
      const data = await lastValueFrom(this.client.send("film/create", {createFilm, image}))
  ...
  @Get('film/search')
	async getBySearch(@Query() searchBy: Search) {
		const personData = await lastValueFrom(this.client.send("person/search", searchBy))
    return await lastValueFrom(this.client.send("film/search", {searchBy, personData}))
	}
  ...
  @Get('film/:id')
  @ApiResponse({ status: 200, description: 'The record has been successfully gate.', schema : {allOf : [{$ref : getSchemaPath(CreateFilm)}]}})
  ...
  @Put("film/edit/:id")
	...
	async editFilm(
		@Param('id') id : number,
		@Body() editFilmDto : EditFIlm
	){
		return await lastValueFrom(this.client.send("film/edit/:id", {id, editFilmDto}))
	}
  ...
  @Put("genre/edit/:id")
    ...
    async editGenre(
        @Param("id") id : number,
        @Body() genreDto : EditGenre
    ){
        return await lastValueFrom(this.client.send("genre/edit/:id", {id, genreDto}))
    }
  ...
  @UseInterceptors(
    ...
    return await lastValueFrom(this.client.send("person/create", {createDto, image}))
	}
  ...
  async getUser(@Body('id') id: number, @Req() request: Request): Promise<any> {
    ...
    return await lastValueFrom(this.client.send("auth/getUser", {id, cookie}))
  }
  ...
  async handleRedirect(@Req() req: Request, @Res({passthrough : true}) response : Response) {
    ...
    let jwt =  await lastValueFrom(this.client.send("auth/google/redirect", user))
    response.cookie("jwt", jwt, {httpOnly : true})
    return user
  }
  ...
  @UseGuards(VKAuthGuard)
  async vkRedirect(@Req() req: Request, @Res({passthrough : true}) response : Response) {
    const user :  any  = req.user;
  ...
  async createCom(@Body() createComment : CreateComment){
    return await lastValueFrom(this.client.send("comment/create", createComment))
